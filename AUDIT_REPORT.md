...
**Status:** Deferred — per OQ-8's resolution, scoped as its own dedicated accessibility project rather than folded into this remediation pass. Not fixed.
...
Everything below was fixed in a same-day remediation pass following the audit and the open-question resolutions. Deferred: H-10 (ARIA) and the broader label-`for`/`id` wiring noted under L-9 — both scoped to their own accessibility project per OQ-8.
